...
export class ExtensionPopup extends LitElement {
    ...
    render() {
        return html`
            <popup-header style='color : #9179FA'> Dark Filter </popup-header>
...
export class FoundPatternsList extends LitElement {
    ...
    render() {
        ...
        <div>
            ...
            <a target="_blank" style="color: red" href=" https://forms.office.com/r/5PdiZw8FYH"> Report this to authorities here!</a>
...
export class ShowPatternButtons extends LitElement {
     ...
     * @param {Event} event
     */
    showPreviousPattern(event) {
        this.showPattern(-1);
    }

    /**
     * Function to show the first pattern element.
     * Used as a click event handler.
     * @param {Event} event
     */
    showFirstPattern(event) {
        this.showPattern(0);
    }

    /**
     * Function to handle a click event on a pattern link.
     * @param {Event} event
     */
    async patternLinkClick(event) {
        // Extract the pattern highlighter ID from the `id` attribute of the clicked link.
        let patternId = parseInt(event.target.id.replace("phid", ""));
        // If the ID could not be extracted, do nothing.
        if (isNaN(patternId)) {
            return;
        }
        // Set the ID of the currently shown element to the clicked ID.
        this._currentPatternId = patternId;
        // Send a message to the content script to show the element with the ID of `_currentPatternId`.
        await brw.tabs.sendMessage((await getCurrentTab()).id, { "showElement": this._currentPatternId });
    }
    ...
    /**
     * From the Lit documentation (https://lit.dev/docs/components/lifecycle/):
     * "Called once before the component is first rendered.".
     * This function is used to extract the visible patterns at the beginning.
     */
    firstUpdated() {
     ...
     * This function is used to set the interval for re-extracting the visible patterns.
     */
    updated(changedProperties) {
        // Set an interval for re-extracting the visible patterns.
        // This is necessary because the content script may inject new elements dynamically.
        setInterval(() => { this.extractVisiblePatterns(); }, 5000);
    }
    ...
    render() {
        ...
        return html`
        <div>
            <h2>${brw.i18n.getMessage("headingShowPatterns")}</h2>
            <span class="button" @click=${this.showFirstPattern}>&lt;&lt;</span>
            <span class="button" @click=${this.showPreviousPattern}>&lt;</span>
            ${this._currentPatternId !== null ? html`
                <a href="#" id="${"phid" + this._currentPatternId}" @click=${this.patternLinkClick} class="button">
                    ${brw.i18n.getMessage("buttonShowInPage")}
                </a>` : html``}
            <span class="button" @click=${this.showNextPattern}>&gt;</span>
            <span class="button" @click=${this.showPattern.bind(this, this._visiblePatterns.length - 1)}>&gt;&gt;</span>
        </div>
      `;
    }
}
...
/**
 * Lit component for the footer of the popup.
 * @extends LitElement
 */
export class PopupFooter extends LitElement {
    ...
    static styles = [
        ...
        css`
            footer {
                ...
                right: 0;
                padding: 0.5em;
                background-color: rgb(143, 92, 255);
                text-align: center;
                font-size: 0.8em;
            }
        `
    ...
    render() {
        return html`
        <footer>
            <p> ${brw.i18n.getMessage("extDescription")}</p>
            <a target="_blank" href=" https://forms.office.com/r/5PdiZw8FYH"> Report this to authorities here!</a>
        </footer>
      `;
    }
}
